...
"""
First attempt caused TLE, brute force just iterating
through all the possible choices

Interesting sequences occurring:

valid_triangles(list(range(1, n))) creates sequence A002623
    Expansion of 1/((1-x)^4*(1+x)).

[valid_triangles([5] * n) for n in range(5, 15)] creates sequence
    A000292 Tetrahedral (or triangular pyramidal) numbers: a(n) = C(n+2,3) = n*(n+1)*(n+2)/6.

valid_triangles(list(range(1, n)) + [n]) creates sequence ...

Something also ties to
A002620 Quarter-squares: a(n) = floor(n/2)*ceiling(n/2). Equivalently, a(n) = floor(n^2/4).

GRAVEYARD
return sum(map(
    is_valid
    , set(combinations(nums, 3))
))
